...
$structureDirs = @($DZSMBaseRoot, $BackupsRoot, $AppRoot, $DocsRoot, $DZSMInstallRoot, $ReportsRoot, $BugReportRoot, $ScriptsRoot, $BugReportRoot, $ArchiveRoot)
...
$allReportFiles = @(
    ...
    $SafetyDocsLogFile,$WorkDriveNotesFile,$LMStudioPolicyFile,$ShortcutLogFile,$BugReportLogFile,$UploadPackageLogFile,$BuildWrapperLogFile,$MemoryPackageLogFile,$GitCommitHelperLogFile,$NextFile
)
...
function Invoke-DZSMSelfCleanup {
    "DeutschZ System Manager v0.4.1 - Selbstbereinigung alter Versionen" | Out-File -FilePath $SelfCleanupFile -Encoding UTF8
    ...
    "v0.4.1 Fix: Erfasst jetzt auch alte BAT/PS1/README/Config/ZIP-Dateien im Arbeitsordner." | Out-File -FilePath $SelfCleanupFile -Append -Encoding UTF8
    ...
    $currentBatName = "Start_DZSM_v0_4_1_MEMORY_COMMIT_PACKAGE.bat"
    $currentPs1Name = "DZSM_v0_4_1_Memory_Commit_Package.ps1"
    $currentKeepNames = @(
        $currentBatName,
        $currentPs1Name,
        "README_START.txt",
...
function Write-DZSMStandardStructureReport {
    "DeutschZ System Manager v0.4.1 - Standardstruktur" | Out-File -FilePath $StructureFile -Encoding UTF8
...
"BugReports: $BugReportRoot" | Out-File -FilePath $StructureFile -Append -Encoding UTF8
"Memory: $MemoryRoot" | Out-File -FilePath $StructureFile -Append -Encoding UTF8
"UploadPackage: $UploadPackageRoot" | Out-File -FilePath $StructureFile -Append -Encoding UTF8
...
"BuildWrapper: $BuildWrapperRoot" | Out-File -FilePath $StructureFile -Append -Encoding UTF8
"GitCommitHelper: $GitCommitHelperRoot" | Out-File -FilePath $StructureFile -Append -Encoding UTF8
    "" | Out-File -FilePath $StructureFile -Append -Encoding UTF8

    foreach ($dir in @($DZSMBaseRoot, $BackupsRoot, $AppRoot, $DocsRoot, $DZSMInstallRoot, $ReportsRoot, $BugReportRoot, $ScriptsRoot, $BugReportRoot, $ArchiveRoot)) {
...
Datum: $(Get-Date)
Aktuelle Version: v0.4.1 MEMORY-COMMIT-PACKAGE
Startdatei: Start_DZSM_v0_4_1_MEMORY_COMMIT_PACKAGE.bat
...
function Write-DZSMPDriveWorkDriveNotes {
    "DeutschZ System Manager v0.4.1 - P-Root / DayZ Tools WorkDrive Notes" | Out-File -FilePath $WorkDriveNotesFile -Encoding UTF8
    ...
    "Entscheidung ab v0.4.1:" | Out-File -FilePath $WorkDriveNotesFile -Append -Encoding UTF8
...
function Invoke-DZSMAutoBackup {
    "DeutschZ System Manager v0.4.1 - SAFE AUTO BACKUP" | Out-File -FilePath $AutoBackupLogFile -Encoding UTF8
...
function Invoke-DZSMGitHubExport {
    "DeutschZ System Manager v0.4.1 - GitHub Export" | Out-File -FilePath $GitHubExportLogFile -Encoding UTF8
...
function Write-DZSMGitHubSafetyDocuments {
    "DeutschZ System Manager v0.4.1 - GitHub Safety Documentation" | Out-File -FilePath $SafetyDocsLogFile -Encoding UTF8
...
Start_DZSM_v0_4_1_MEMORY_COMMIT_PACKAGE.bat
...
function Write-DZSMLMStudioPolicy {
    "DeutschZ System Manager v0.4.1 - LM Studio / Modell Policy" | Out-File -FilePath $LMStudioPolicyFile -Encoding UTF8
    ...
    "Entscheidung ab v0.4.1:" | Out-File -FilePath $LMStudioPolicyFile -Append -Encoding UTF8
...
function Update-DZSMDesktopShortcuts {
    ...
    "DeutschZ System Manager v0.4.1 - Desktop-Verknuepfungen" | Out-File -FilePath $ShortcutLogFile -Encoding UTF8
    "Datum: $(Get-Date)" | Out-File -FilePath $ShortcutLogFile -Append -Encoding UTF8
    "Regel: Benutzerdefinierte Desktop-Verknuepfungen werden NICHT geloescht." | Out-File -FilePath $ShortcutLogFile -Append -Encoding UTF8
    "Wichtig: DeutschZ_System_Manager - Verknuepfung.lnk bleibt erhalten." | Out-File -FilePath $ShortcutLogFile -Append -Encoding UTF8
    "v0.4.1: Eindeutige NeonGreen-v040 Icon-Dateinamen gegen Windows Icon-Cache." | Out-File -FilePath $ShortcutLogFile -Append -Encoding UTF8
    ...
    $currentBat = Join-Path $PSScriptRoot "Start_DZSM_v0_4_1_MEMORY_COMMIT_PACKAGE.bat"

    $iconStart = Join-Path $PSScriptRoot "icons\DZSM_Start_NeonGreen_v040.ico"
    $iconUpload = Join-Path $PSScriptRoot "icons\DZSM_Upload_NeonGreen_v040.ico"
    $iconReports = Join-Path $PSScriptRoot "icons\DZSM_Reports_NeonGreen_v040.ico"
    $iconBuild = Join-Path $PSScriptRoot "icons\DZSM_Build_NeonGreen_v040.ico"

    $autoLinkNamesToReplace = @(
    ...
    foreach ($linkName in $autoLinkNamesToReplace) {
        $lnkPath = Join-Path $desktop $linkName
        if (Test-PathFast $lnkPath) {
            try {
                Remove-Item -Path $lnkPath -Force -ErrorAction Stop
                "OK alte Auto-Verknuepfung ersetzt/entfernt: $lnkPath" | Out-File -FilePath $ShortcutLogFile -Append -Encoding UTF8
            } catch {
                "WARNUNG Auto-Verknuepfung konnte nicht entfernt werden: $lnkPath : $($_.Exception.Message)" | Out-File -FilePath $ShortcutLogFile -Append -Encoding UTF8
            }
        }
    }

    $protectedUserLink = Join-Path $desktop "DeutschZ_System_Manager - Verknuepfung.lnk"
    if (Test-PathFast $protectedUserLink) {
        "GESCHUETZT/BEHALTEN: $protectedUserLink" | Out-File -FilePath $ShortcutLogFile -Append -Encoding UTF8
    }
    ...
    try {
        ...
        if (Test-PathFast $currentBat) {
            ...
            if (Test-PathFast $iconStart) { $batShortcut.IconLocation = $iconStart }
            $batShortcut.Save()
            "OK Start-Verknuepfung erstellt." | Out-File -FilePath $ShortcutLogFile -Append -Encoding UTF8
        }

        if (Test-PathFast $UploadPackageRoot) {
            $uploadShortcut = $shell.CreateShortcut((Join-Path $desktop "DeutschZ System Manager - Upload Reports.lnk"))
            $uploadShortcut.TargetPath = $UploadPackageRoot
            $uploadShortcut.WorkingDirectory = $UploadPackageRoot
            $uploadShortcut.Description = "Ordner mit genau den DZSM-Reports, die an ChatGPT hochgeladen werden sollen"
            if (Test-PathFast $iconUpload) { $uploadShortcut.IconLocation = $iconUpload }
            $uploadShortcut.Save()
            "OK Upload-Reports-Verknuepfung erstellt." | Out-File -FilePath $ShortcutLogFile -Append -Encoding UTF8
        }

        if (Test-PathFast $ReportsRoot) {
            $reportsShortcut = $shell.CreateShortcut((Join-Path $desktop "DeutschZ System Manager - Reports.lnk"))
            $reportsShortcut.TargetPath = $ReportsRoot
            $reportsShortcut.WorkingDirectory = $ReportsRoot
            $reportsShortcut.Description = "DZSM Haupt-Reportordner"
            if (Test-PathFast $iconReports) { $reportsShortcut.IconLocation = $iconReports }
            $reportsShortcut.Save()
            "OK Reports-Verknuepfung erstellt." | Out-File -FilePath $ShortcutLogFile -Append -Encoding UTF8
        }

        $buildBat = Join-Path $BuildWrapperRoot "DeutschZ_Build_Wrapper_CheckOnly.bat"
        if (Test-PathFast $buildBat) {
            $buildShortcut = $shell.CreateShortcut((Join-Path $desktop "DeutschZ Build Wrapper - Check.lnk"))
            $buildShortcut.TargetPath = $buildBat
            $buildShortcut.WorkingDirectory = $BuildWrapperRoot
            $buildShortcut.Description = "DeutschZ Build Wrapper CheckOnly"
            if (Test-PathFast $iconBuild) { $buildShortcut.IconLocation = $iconBuild }
            $buildShortcut.Save()
            "OK Build-Wrapper-Verknuepfung erstellt." | Out-File -FilePath $ShortcutLogFile -Append -Encoding UTF8
        }
        ...
        try {
            ...
            "OK Icon-Refresh angestossen: ie4uinit.exe -show" | Out-File -FilePath $ShortcutLogFile -Append -Encoding UTF8
        } catch {}
    } catch {
...
function Update-DZSMBugReportWorkspace {
    "DeutschZ System Manager v0.4.1 - BugReport Arbeitsordner" | Out-File -FilePath $BugReportLogFile -Encoding UTF8
    "Datum: $(Get-Date)" | Out-File -FilePath $BugReportLogFile -Append -Encoding UTF8
    "BugReportRoot: $BugReportRoot" | Out-File -FilePath $BugReportLogFile -Append -Encoding UTF8
    "" | Out-File -FilePath $BugReportLogFile -Append -Encoding UTF8

    New-Item -ItemType Directory -Path $BugReportRoot -Force | Out-Null
    New-Item -ItemType Directory -Path (Join-Path $BugReportRoot "offen") -Force | Out-Null
    New-Item -ItemType Directory -Path (Join-Path $BugReportRoot "geloest") -Force | Out-Null
    New-Item -ItemType Directory -Path (Join-Path $BugReportRoot "notizen") -Force | Out-Null
    New-Item -ItemType Directory -Path (Join-Path $BugReportRoot "logs") -Force | Out-Null

    $currentBugFile = Join-Path $BugReportRoot "BUGREPORT_AKTUELL.md"
    $lessonsFile = Join-Path $BugReportRoot "LOESUNGSWEGE_UND_VERMEIDUNG.md"
    $knownIssuesFile = Join-Path $BugReportRoot "KNOWN_ISSUES.md"

    if (-not (Test-PathFast $currentBugFile)) {
@"
# DZSM BugReport aktuell

Dieses Dokument sammelt Fehler, Beobachtungen und offene Punkte zum DeutschZ System Manager.

## Aktuelle offene Fehler

| Datum | Version | Fehler/Beschreibung | Ursache/Vermutung | Loesungsweg | Status |
|---|---|---|---|---|---|
| $(Get-Date -Format "yyyy-MM-dd") | v0.4.1 | Desktop-Verknuepfung wurde zu breit geloescht | Wildcard-Loeschregel fuer DZSM-Links war zu aggressiv | Nur exakt bekannte Auto-Linknamen ersetzen; benutzerdefinierte Links schuetzen | behoben |
| $(Get-Date -Format "yyyy-MM-dd") | v0.3.6 | Archivordner wurde nicht vollstaendig gefuellt | Alte Dateimuster wurden zu eng erkannt | BAT/PS1/README/Config/ZIP/alte Versionsordner gezielter archivieren | in v0.3.6 verbessert |

## Neue Fehler hier eintragen

| Datum | Version | Fehler/Beschreibung | Ursache/Vermutung | Loesungsweg | Status |
|---|---|---|---|---|---|
"@ | Out-File -FilePath $currentBugFile -Encoding UTF8
        "OK erstellt: $currentBugFile" | Out-File -FilePath $BugReportLogFile -Append -Encoding UTF8
    } else {
        "OK vorhanden: $currentBugFile" | Out-File -FilePath $BugReportLogFile -Append -Encoding UTF8
    }

    if (-not (Test-PathFast $lessonsFile)) {
@"
...
"@ | Out-File -FilePath $lessonsFile -Encoding UTF8
        "OK erstellt: $lessonsFile" | Out-File -FilePath $BugReportLogFile -Append -Encoding UTF8
    } else {
        "OK vorhanden: $lessonsFile" | Out-File -FilePath $BugReportLogFile -Append -Encoding UTF8
    }

    if (-not (Test-PathFast $knownIssuesFile)) {
@"
# DZSM Known Issues

## Bekannte Punkte

1. P:\Root enthaelt DayZ Tools WorkDrive/Vanilla-Daten und Legacy-Mischdateien.
   - Nicht automatisch loeschen/verschieben/exportieren.
   - Nur dokumentieren.

2. ReadyMods ist Build-/PBO-Ausgabe.
   - Nicht als GitHub-Quelle behandeln.

3. LM Studio ist optional/experimentell.
   - Nicht automatisch sichern/exportieren.

4. Desktop-Verknuepfungen:
   - Benutzerdefinierte Verknuepfungen schuetzen.
   - Nur exakt bekannte DZSM-Auto-Verknuepfungen ersetzen.

5. Archivierung:
   - Alte DZSM-Dateien archivieren, nicht loeschen.
"@ | Out-File -FilePath $knownIssuesFile -Encoding UTF8
        "OK erstellt: $knownIssuesFile" | Out-File -FilePath $BugReportLogFile -Append -Encoding UTF8
    } else {
        "OK vorhanden: $knownIssuesFile" | Out-File -FilePath $BugReportLogFile -Append -Encoding UTF8
    }

    $runNote = Join-Path (Join-Path $BugReportRoot "logs") ("BUGREPORT_RUN_" + (Get-Date -Format "yyyy-MM-dd_HH-mm-ss") + ".txt")
@"
DZSM BugReport Laufnotiz
========================

Datum: $(Get-Date)
Version: v0.4.1 MEMORY-COMMIT-PACKAGE
PC: $env:COMPUTERNAME
Benutzer: $env:USERNAME

Gepruefte Regeln:
- Desktop-Verknuepfung DeutschZ_System_Manager - Verknuepfung.lnk darf nicht geloescht werden.
- Nur exakt bekannte Auto-Links werden ersetzt.
- BugReport-Arbeitsordner existiert.
- Fehler und Loesungswege werden dokumentiert.
"@ | Out-File -FilePath $runNote -Encoding UTF8

    "OK Laufnotiz erstellt: $runNote" | Out-File -FilePath $BugReportLogFile -Append -Encoding UTF8
}




function Update-DZSMBuildWrapper {
    "DeutschZ System Manager v0.4.1 - Build Wrapper Setup" | Out-File -FilePath $BuildWrapperLogFile -Encoding UTF8
    "Datum: $(Get-Date)" | Out-File -FilePath $BuildWrapperLogFile -Append -Encoding UTF8
    "BuildWrapperRoot: $BuildWrapperRoot" | Out-File -FilePath $BuildWrapperLogFile -Append -Encoding UTF8
    "Regel: Wrapper wird im sauberen GitHub-Projekt erstellt. P:\Root bleibt DayZ Tools WorkDrive/Legacy." | Out-File -FilePath $BuildWrapperLogFile -Append -Encoding UTF8
    "" | Out-File -FilePath $BuildWrapperLogFile -Append -Encoding UTF8

    if (-not (Test-PathFast "P:\")) {
        "SKIP: P:\ ist nicht aktiv. Build Wrapper wird nicht erstellt." | Out-File -FilePath $BuildWrapperLogFile -Append -Encoding UTF8
        return
    }

    if (-not (Test-PathFast $DeutschZRepoRoot)) {
        "SKIP: DeutschZRepoRoot fehlt: $DeutschZRepoRoot" | Out-File -FilePath $BuildWrapperLogFile -Append -Encoding UTF8
        return
    }
    ...
    New-Item -ItemType Directory -Path (Join-Path $BuildWrapperRoot "docs") -Force | Out-Null

    $wrapperPs1Path = Join-Path $BuildWrapperRoot "DeutschZ_Build_Wrapper.ps1"
    $wrapperBatPath = Join-Path $BuildWrapperRoot "DeutschZ_Build_Wrapper_CheckOnly.bat"
    $wrapperConfigPath = Join-Path $BuildWrapperRoot "BUILD_CONFIG.json"
    $wrapperReadmePath = Join-Path $BuildWrapperRoot "README_BUILD_WRAPPER.md"
...
$config = [ordered]@{
    ...
    repo_root = "P:\GitHub_DeutschZModZ\DeutschZModZ"
    mods_root = "P:\GitHub_DeutschZModZ\DeutschZModZ\mods"
    readymods_root = "P:\ReadyMods"
    active_mod_order = @(
        "DeutschZ_Core",
        "DeutschZ_ExpansionBridge",
        "DeutschZ_KotHZ",
    ...
    rules = @{
        ...
        check_p_drive_readiness = $true
        expansion_bridge_only_direct_expansion_layer = $true
    }
}
...
"@
    ...
    "OK Build Wrapper PS1: $wrapperPs1Path" | Out-File -FilePath $BuildWrapperLogFile -Append -Encoding UTF8
    "OK Build Wrapper BAT: $wrapperBatPath" | Out-File -FilePath $BuildWrapperLogFile -Append -Encoding UTF8
    "OK Build Wrapper Config: $wrapperConfigPath" | Out-File -FilePath $BuildWrapperLogFile -Append -Encoding UTF8
    "OK Build Wrapper README: $wrapperReadmePath" | Out-File -FilePath $BuildWrapperLogFile -Append -Encoding UTF8
}



function Update-DZSMMemoryPackage {
    "DeutschZ System Manager v0.4.1 - ChatGPT Memory Package" | Out-File -FilePath $MemoryPackageLogFile -Encoding UTF8
    "Datum: $(Get-Date)" | Out-File -FilePath $MemoryPackageLogFile -Append -Encoding UTF8
    "MemoryRoot: $MemoryRoot" | Out-File -FilePath $MemoryPackageLogFile -Append -Encoding UTF8
    "Regel: Diese Dateien helfen nach Chat-Loeschung, den Projektstand wiederherzustellen." | Out-File -FilePath $MemoryPackageLogFile -Append -Encoding UTF8
    "" | Out-File -FilePath $MemoryPackageLogFile -Append -Encoding UTF8

    New-Item -ItemType Directory -Path $MemoryRoot -Force | Out-Null

    $memoryFile = Join-Path $MemoryRoot "CHATGPT_ERINNERUNG_AKTUELL.md"
    $memoryStatusFile = Join-Path $MemoryRoot "MEMORY_STATUS.json"
...
'@
    ...
    $status = [ordered]@{
        ...
        user_note = "Patrick will eventuell alte Chats loeschen; diese Datei ins Uploadpaket legen und bei Bedarf neu in Memory speichern."
        secrets_included = $false
        should_store_if_missing = $true
        should_not_store_passwords_or_keys = $true
    }
    $status | ConvertTo-Json -Depth 5 | Out-File -FilePath $memoryStatusFile -Encoding UTF8

    "OK Memory-Datei erstellt: $memoryFile" | Out-File -FilePath $MemoryPackageLogFile -Append -Encoding UTF8
    "OK Memory-Status erstellt: $memoryStatusFile" | Out-File -FilePath $MemoryPackageLogFile -Append -Encoding UTF8

    try {
        if (Test-PathFast $UploadPackageRoot) {
            Copy-Item -Path $memoryFile -Destination (Join-Path $UploadPackageRoot "CHATGPT_ERINNERUNG_AKTUELL.md") -Force -ErrorAction SilentlyContinue
            Copy-Item -Path $memoryStatusFile -Destination (Join-Path $UploadPackageRoot "MEMORY_STATUS.json") -Force -ErrorAction SilentlyContinue
            "OK Memory-Dateien ins Uploadpaket kopiert." | Out-File -FilePath $MemoryPackageLogFile -Append -Encoding UTF8
        }
    } catch {
        "WARNUNG: Memory-Dateien konnten nicht ins Uploadpaket kopiert werden: $($_.Exception.Message)" | Out-File -FilePath $MemoryPackageLogFile -Append -Encoding UTF8
    }

    try {
        if (Test-PathFast $DeutschZRepoRoot) {
            $repoMemoryRoot = Join-Path $DeutschZRepoRoot "DeutschZ_SystemManager_Tool_Dokumentation\memory"
            New-Item -ItemType Directory -Path $repoMemoryRoot -Force | Out-Null
            Copy-Item -Path $memoryFile -Destination (Join-Path $repoMemoryRoot "CHATGPT_ERINNERUNG_AKTUELL.md") -Force -ErrorAction SilentlyContinue
            Copy-Item -Path $memoryStatusFile -Destination (Join-Path $repoMemoryRoot "MEMORY_STATUS.json") -Force -ErrorAction SilentlyContinue
            "OK Memory-Dateien ins Repo-Dokumentationsverzeichnis kopiert: $repoMemoryRoot" | Out-File -FilePath $MemoryPackageLogFile -Append -Encoding UTF8
        }
    } catch {
        "WARNUNG: Memory-Dateien konnten nicht ins Repo kopiert werden: $($_.Exception.Message)" | Out-File -FilePath $MemoryPackageLogFile -Append -Encoding UTF8
    }
}



function Update-DZSMGitCommitHelper {
    "DeutschZ System Manager v0.4.1 - Git Commit Helper" | Out-File -FilePath $GitCommitHelperLogFile -Encoding UTF8
    "Datum: $(Get-Date)" | Out-File -FilePath $GitCommitHelperLogFile -Append -Encoding UTF8
    "GitCommitHelperRoot: $GitCommitHelperRoot" | Out-File -FilePath $GitCommitHelperLogFile -Append -Encoding UTF8
    "" | Out-File -FilePath $GitCommitHelperLogFile -Append -Encoding UTF8

    if (-not (Test-PathFast $DeutschZRepoRoot)) {
        "SKIP: DeutschZRepoRoot fehlt: $DeutschZRepoRoot" | Out-File -FilePath $GitCommitHelperLogFile -Append -Encoding UTF8
        return
    }

    New-Item -ItemType Directory -Path $GitCommitHelperRoot -Force | Out-Null

    $commitMessageFile = Join-Path $GitCommitHelperRoot "DZSM_GIT_COMMIT_MESSAGE.txt"
    $commitBat = Join-Path $GitCommitHelperRoot "GIT_COMMIT_DZSM_CHECK_AND_COMMIT.bat"
    $commitPs1 = Join-Path $GitCommitHelperRoot "GIT_COMMIT_DZSM_CHECK_AND_COMMIT.ps1"
    $readme = Join-Path $GitCommitHelperRoot "README_GIT_COMMIT_HELPER.md"
...
pause
'@
    $commitPs1Content | Out-File -FilePath $commitPs1 -Encoding UTF8
...
"@
    $commitBatContent | Out-File -FilePath $commitBat -Encoding ASCII

$readmeContent = @"
# DeutschZ Git Commit Helper
...
Dieser Helfer bereitet einen sauberen Commit fuer DZSM, Safety-Dokumentation, Memory-Paket, Uploadpaket und Build Wrapper vor.

## Start

```text
GIT_COMMIT_DZSM_CHECK_AND_COMMIT.bat
```

## Wichtig

Der Helper pusht nicht automatisch.

Vor dem Commit pruefen:

- keine Backups im Repo
- keine privaten Reports
- keine Secrets
- keine LM Studio Modelle
- kein P:\Root Export
- keine ReadyMods/PBO-Ausgabe
- keine Workshopdateien veraendert, ausser Patrick hat es explizit beauftragt
...
```
"@
    $readmeContent | Out-File -FilePath $readme -Encoding UTF8

    "OK Commit Message: $commitMessageFile" | Out-File -FilePath $GitCommitHelperLogFile -Append -Encoding UTF8
    "OK Commit PS1: $commitPs1" | Out-File -FilePath $GitCommitHelperLogFile -Append -Encoding UTF8
    "OK Commit BAT: $commitBat" | Out-File -FilePath $GitCommitHelperLogFile -Append -Encoding UTF8
    "OK README: $readme" | Out-File -FilePath $GitCommitHelperLogFile -Append -Encoding UTF8

    try {
        Copy-Item -Path $commitMessageFile -Destination (Join-Path $UploadPackageRoot "DZSM_GIT_COMMIT_MESSAGE.txt") -Force -ErrorAction SilentlyContinue
        "OK Commit Message ins Uploadpaket kopiert." | Out-File -FilePath $GitCommitHelperLogFile -Append -Encoding UTF8
    } catch {
        "WARNUNG: Commit Message konnte nicht ins Uploadpaket kopiert werden: $($_.Exception.Message)" | Out-File -FilePath $GitCommitHelperLogFile -Append -Encoding UTF8
    }
}


function Update-DZSMUploadPackage {
    "DeutschZ System Manager v0.4.1 - Upload-Paket fuer ChatGPT" | Out-File -FilePath $UploadPackageLogFile -Encoding UTF8
    "Datum: $(Get-Date)" | Out-File -FilePath $UploadPackageLogFile -Append -Encoding UTF8
    "UploadPackageRoot: $UploadPackageRoot" | Out-File -FilePath $UploadPackageLogFile -Append -Encoding UTF8
    "Regel: Der volle reports-Ordner bleibt lokal. Fuer ChatGPT wird nur ein kleiner Upload-Ordner gepflegt." | Out-File -FilePath $UploadPackageLogFile -Append -Encoding UTF8
    "" | Out-File -FilePath $UploadPackageLogFile -Append -Encoding UTF8

    New-Item -ItemType Directory -Path $UploadPackageRoot -Force | Out-Null

    # Alte Upload-Paket-Dateien entfernen, aber nur in diesem eigenen Upload-Ordner.
    try {
        Get-ChildItem -Path $UploadPackageRoot -File -ErrorAction SilentlyContinue | Remove-Item -Force -ErrorAction SilentlyContinue
    } catch {
        "WARNUNG: UploadPackageRoot konnte nicht bereinigt werden: $($_.Exception.Message)" | Out-File -FilePath $UploadPackageLogFile -Append -Encoding UTF8
    }
    ...
    foreach ($fileName in $wantedFiles) {
        $src = Join-Path $ReportsRoot $fileName
        $dst = Join-Path $UploadPackageRoot $fileName
        if (Test-PathFast $src) {
            try {
                Copy-Item -Path $src -Destination $dst -Force -ErrorAction Stop
                "OK kopiert: $fileName" | Out-File -FilePath $UploadPackageLogFile -Append -Encoding UTF8
            } catch {
                "FEHLER beim Kopieren $fileName : $($_.Exception.Message)" | Out-File -FilePath $UploadPackageLogFile -Append -Encoding UTF8
            }
        } else {
            "FEHLT/OPTIONAL: $fileName" | Out-File -FilePath $UploadPackageLogFile -Append -Encoding UTF8
        }
    }

    $readme = @"
DZSM Upload-Paket fuer ChatGPT
==============================

Dieser Ordner enthaelt nur die wichtigsten DZSM-Dateien, die Patrick bei Bedarf hochladen soll.

Der volle reports-Ordner bleibt lokal unter:
$ReportsRoot

Dieser Upload-Ordner liegt unter:
$UploadPackageRoot

Bitte bei Fehleranalyse diesen ganzen Ordner oder die einzelnen Dateien daraus hochladen.

Nicht notwendig fuer normalen Upload:
- komplette Backups
- komplette Robocopy-Detail-Logs
- private Dateien
- P:\Root
- LM Studio Modelle
- ReadyMods/PBO-Ausgaben

Letzte Aktualisierung:
$(Get-Date)
"@
    $readme | Out-File -FilePath (Join-Path $UploadPackageRoot "README_UPLOAD_HINWEIS.txt") -Encoding UTF8
    "OK erstellt: README_UPLOAD_HINWEIS.txt" | Out-File -FilePath $UploadPackageLogFile -Append -Encoding UTF8


    # Memory- und Commit-Dateien liegen nicht alle im ReportsRoot, daher hier explizit nachziehen.
    try {
        $memoryFile = Join-Path $MemoryRoot "CHATGPT_ERINNERUNG_AKTUELL.md"
        $memoryStatusFile = Join-Path $MemoryRoot "MEMORY_STATUS.json"
        $commitMessageFile = Join-Path $GitCommitHelperRoot "DZSM_GIT_COMMIT_MESSAGE.txt"
        if (Test-PathFast $memoryFile) { Copy-Item -Path $memoryFile -Destination (Join-Path $UploadPackageRoot "CHATGPT_ERINNERUNG_AKTUELL.md") -Force -ErrorAction SilentlyContinue }
        if (Test-PathFast $memoryStatusFile) { Copy-Item -Path $memoryStatusFile -Destination (Join-Path $UploadPackageRoot "MEMORY_STATUS.json") -Force -ErrorAction SilentlyContinue }
        if (Test-PathFast $commitMessageFile) { Copy-Item -Path $commitMessageFile -Destination (Join-Path $UploadPackageRoot "DZSM_GIT_COMMIT_MESSAGE.txt") -Force -ErrorAction SilentlyContinue }
        "OK Memory-/Commit-Dateien explizit ins Uploadpaket kopiert." | Out-File -FilePath $UploadPackageLogFile -Append -Encoding UTF8
    } catch {
        "WARNUNG: Memory-/Commit-Dateien konnten nicht explizit kopiert werden: $($_.Exception.Message)" | Out-File -FilePath $UploadPackageLogFile -Append -Encoding UTF8
    }

    # ZIP optional im gleichen Ordner erstellen
    try {
        $zipPath = Join-Path $UploadPackageRoot "DZSM_UPLOAD_PACKAGE.zip"
        if (Test-PathFast $zipPath) { Remove-Item $zipPath -Force -ErrorAction SilentlyContinue }
        Compress-Archive -Path (Join-Path $UploadPackageRoot "*") -DestinationPath $zipPath -Force -ErrorAction SilentlyContinue
        if (Test-PathFast $zipPath) {
            "OK ZIP erstellt: $zipPath" | Out-File -FilePath $UploadPackageLogFile -Append -Encoding UTF8
        }
    } catch {
        "WARNUNG: Upload-ZIP konnte nicht erstellt werden: $($_.Exception.Message)" | Out-File -FilePath $UploadPackageLogFile -Append -Encoding UTF8
    }
}
...
# START
# ------------------------------------------------------------
"DeutschZ System Manager v0.4.1 - DayZ Tools / P-Drive Mount Log" | Out-File -FilePath $MountLogFile -Encoding UTF8
...
try { Update-DZSMBugReportWorkspace } catch { "BugReport Arbeitsordner fehlgeschlagen: $($_.Exception.Message)" | Out-File -FilePath $BugReportLogFile -Append -Encoding UTF8 }
...
try { Update-DZSMBuildWrapper } catch { "Build Wrapper Setup fehlgeschlagen: $($_.Exception.Message)" | Out-File -FilePath $BuildWrapperLogFile -Append -Encoding UTF8 }
try { Update-DZSMMemoryPackage } catch { "Memory-Paket fehlgeschlagen: $($_.Exception.Message)" | Out-File -FilePath $MemoryPackageLogFile -Append -Encoding UTF8 }
try { Update-DZSMGitCommitHelper } catch { "Git Commit Helper fehlgeschlagen: $($_.Exception.Message)" | Out-File -FilePath $GitCommitHelperLogFile -Append -Encoding UTF8 }

# ------------------------------------------------------------
# HAUPTREPORT
# ------------------------------------------------------------
"DeutschZ System Manager v0.4.1 MEMORY-COMMIT-PACKAGE - Inventurbericht" | Out-File -FilePath $ReportFile -Encoding UTF8
...
$knownPaths = @(
    $DZSMBaseRoot, $BackupsRoot, $AppRoot, $DocsRoot, $DZSMInstallRoot, $ReportsRoot, $BugReportRoot, $ScriptsRoot,
...
# Backup Plan
"DeutschZ System Manager v0.4.1 - Backup Plan" | Out-File -FilePath $BackupPlanFile -Encoding UTF8
...
"DeutschZ System Manager v0.4.1 - DeutschZ Structure Check" | Out-File -FilePath $DeutschZFile -Encoding UTF8
...
"DeutschZ System Manager v0.4.1 - Cleanup Risk Check FAST" | Out-File -FilePath $CleanupFile -Encoding UTF8
...
# Migration check
"DeutschZ System Manager v0.4.1 - Windows SSD Migration Check" | Out-File -FilePath $MigrationFile -Encoding UTF8
...
foreach ($path in @($DZSMBaseRoot, $BackupsRoot, $ReportsRoot, $BugReportRoot, $DeutschZRepoRoot, "$DeutschZRepoRoot\mods", "P:\ReadyMods", "F:\Programme\steamapps\common\DayZ", "$env:USERPROFILE\Documents\Codex")) {
...
try {
    $summary = [PSCustomObject]@{
        ...
        DesktopShortcutPolicy = "Create current BAT/PS1 desktop shortcuts; remove only exact known auto-link names; protect custom user shortcuts"
        BugReportRoot = $BugReportRoot
        BugReportPolicy = "Create workspace for bug reports, known issues, solution paths and prevention notes"
        UploadPackageRoot = $UploadPackageRoot
        UploadPackagePolicy = "Copy only the reports ChatGPT needs into a dedicated upload folder"
        CustomIcons = $true
        BuildWrapperRoot = $BuildWrapperRoot
        BuildWrapperPolicy = "Create check-only DeutschZ build wrapper under tools/build_wrapper; real build locked by config"
        MemoryRoot = $MemoryRoot
        MemoryPolicy = "Create ChatGPT memory transfer file and status marker; copy into upload package"
        GitCommitHelperRoot = $GitCommitHelperRoot
        GitCommitPolicy = "Create manual git commit helper and commit message; no automatic push"
        NeonGreenIconPolicy = "Elegant dark graphite icons with neon green accents, no red"
        DesktopPS1Shortcut = $false
...
try { Update-DZSMUploadPackage } catch { "Upload-Paket fehlgeschlagen: $($_.Exception.Message)" | Out-File -FilePath $UploadPackageLogFile -Append -Encoding UTF8 }
try { Update-DZSMDesktopShortcuts -ShortcutLogFile $ShortcutLogFile } catch { "Desktop-Verknuepfungen Final-Refresh fehlgeschlagen: $($_.Exception.Message)" | Out-File -FilePath $ShortcutLogFile -Append -Encoding UTF8 }
...
5. 22_P_ROOT_WORKDRIVE_NOTES.txt`r`n6. 23_LMSTUDIO_MODEL_POLICY.txt`r`n7. 24_DESKTOP_SHORTCUTS_LOG.txt`r`n8. 25_BUGREPORT_WORKSPACE_LOG.txt`r`n9. 26_UPLOAD_PACKAGE_LOG.txt`r`n10. 27_BUILD_WRAPPER_LOG.txt`r`n11. 28_MEMORY_PACKAGE_LOG.txt`r`n12. 29_GIT_COMMIT_HELPER_LOG.txt`r`n13. CHATGPT_ERINNERUNG_AKTUELL.md`r`n14. MEMORY_STATUS.json`r`n15. DZSM_GIT_COMMIT_MESSAGE.txt`r`n`r`nOder einfacher: Den Ordner _UPLOAD_FUER_CHATGPT hochladen.
6. 11_DEUTSCHZ_STRUCTURE_CHECK.txt
"@ | Out-File -FilePath $NextFile -Encoding UTF8

Write-Section "ERGEBNIS"
"v0.4.1 fertig." | Out-File -FilePath $ReportFile -Append -Encoding UTF8
