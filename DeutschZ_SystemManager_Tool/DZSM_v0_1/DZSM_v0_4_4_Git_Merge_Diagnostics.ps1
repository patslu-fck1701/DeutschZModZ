...
# DeutschZ System Manager v0.4.4 GIT-MERGE-DIAGNOSTICS
...
function Invoke-SelfCleanup {
    "DeutschZ System Manager v0.4.4 - Selbstbereinigung" | Out-File $SelfCleanupFile -Encoding UTF8
    ...
    $keep = @("Start_DZSM_v0_4_4_GIT_MERGE_DIAGNOSTICS.bat","DZSM_v0_4_4_Git_Merge_Diagnostics.ps1","README_START.txt","DZSM_Config.json")
    $candidates = @()
...
function Write-Structure {
    "DeutschZ System Manager v0.4.4 - Standardstruktur" | Out-File $StructureFile -Encoding UTF8
    foreach ($line in @(
...
==============================================
Datum: $(Get-Date)
Aktuelle Version: v0.4.4 GIT-MERGE-DIAGNOSTICS
Startdatei: Start_DZSM_v0_4_4_GIT_MERGE_DIAGNOSTICS.bat
...
function Write-WorkDriveNotes {
    "DeutschZ System Manager v0.4.4 - P-Root / WorkDrive Notes" | Out-File $WorkDriveNotesFile -Encoding UTF8
...
function Write-LMStudioPolicy {
    "DeutschZ System Manager v0.4.4 - LM Studio / Modell Policy" | Out-File $LMStudioPolicyFile -Encoding UTF8
...
function Update-BugReports {
    "DeutschZ System Manager v0.4.4 - BugReport Arbeitsordner" | Out-File $BugReportLogFile -Encoding UTF8
...
function Invoke-Backup {
    "DeutschZ System Manager v0.4.4 - SAFE AUTO BACKUP" | Out-File $BackupLogFile -Encoding UTF8
...
function Update-GitHubExport {
    "DeutschZ System Manager v0.4.4 - GitHub Export" | Out-File $GitHubExportLogFile -Encoding UTF8
...
function Update-SafetyDocs {
    "DeutschZ System Manager v0.4.4 - GitHub Safety Docs" | Out-File $SafetyDocsLogFile -Encoding UTF8
    ...
    "# Changelog`nv0.4.4 GIT-MERGE-DIAGNOSTICS" | Out-File (Join-Path $GitHubSafetyDocsRoot "CHANGELOG_DZSM.md") -Encoding UTF8
    Append-Line $SafetyDocsLogFile "OK Safety Docs erstellt."
}

function Update-BuildWrapper {
    "DeutschZ System Manager v0.4.4 - Build Wrapper" | Out-File $BuildWrapperLogFile -Encoding UTF8
...
function Update-Memory {
    "DeutschZ System Manager v0.4.4 - Memory Package" | Out-File $MemoryPackageLogFile -Encoding UTF8
...
"@ | Out-File $memoryFile -Encoding UTF8
    [ordered]@{memory_status="AKTUALISIERT";memory_action_for_chatgpt="NEU_SPEICHERN_WENN_DIESER_STAND_FEHLT_ODER_ABWEICHT";version="DZSM v0.4.4 GIT-MERGE-DIAGNOSTICS";date=(Get-Date).ToString("s");secrets_included=$false;should_store_if_missing=$true} | ConvertTo-Json | Out-File $statusFile -Encoding UTF8
...
function Update-GitCommitHelper {
    "DeutschZ System Manager v0.4.4 - Git Commit Helper" | Out-File $GitCommitHelperLogFile -Encoding UTF8
...
function Update-DeutschZStructureReport {
    "DeutschZ System Manager v0.4.4 - DeutschZ Structure Check" | Out-File $DeutschZFile -Encoding UTF8
...
function Update-MainReport {
    "DeutschZ System Manager v0.4.4 GIT-MERGE-DIAGNOSTICS - Inventurbericht" | Out-File $ReportFile -Encoding UTF8
    foreach ($line in @(
        "PC: $env:COMPUTERNAME",
...
function Update-Shortcuts {
    "DeutschZ System Manager v0.4.4 - Desktop Shortcuts / Oberordner" | Out-File $ShortcutLogFile -Encoding UTF8
    ...
    # Nur exakt bekannte Auto-Links entfernen. Keine Wildcards.
    $desktopAutoLinks = @(
    ...
    $iconStart = Join-Path $PSScriptRoot "icons\DZSM_Start_DarkGlass_Green_v043.ico"
    $iconFolder = Join-Path $PSScriptRoot "icons\DZSM_Folder_DarkGlass_Green_v043.ico"
    $iconUpload = Join-Path $PSScriptRoot "icons\DZSM_Upload_DarkGlass_Green_v043.ico"
    $iconReports = Join-Path $PSScriptRoot "icons\DZSM_Reports_DarkGlass_Green_v043.ico"
    $iconBuild = Join-Path $PSScriptRoot "icons\DZSM_Build_DarkGlass_Green_v043.ico"

    # Desktop: nur Start-Link
    $startTarget = Join-Path $PSScriptRoot "Start_DZSM_v0_4_4_GIT_MERGE_DIAGNOSTICS.bat"
    if (Test-PathFast $startTarget) {
        $s = $shell.CreateShortcut((Join-Path $desktop "DeutschZ System Manager - Start.lnk"))
        $s.TargetPath = $startTarget
        $s.WorkingDirectory = $PSScriptRoot
        $s.Description = "Startet den DeutschZ System Manager"
...
"@ | Out-File -FilePath $desktopIni -Encoding ASCII
    ...
    # Oberordner: die anderen 3 Links
    if (Test-PathFast $UploadPackageRoot) {
        $u = $shell.CreateShortcut((Join-Path $desktopFolder "Upload Reports.lnk"))
        ...
        $u.Save()
        Append-Line $ShortcutLogFile "OK Ordner-Link erstellt: Upload Reports"
    }

    if (Test-PathFast $ReportsRoot) {
        $r = $shell.CreateShortcut((Join-Path $desktopFolder "Reports.lnk"))
        $r.TargetPath = $ReportsRoot
        $r.WorkingDirectory = $ReportsRoot
        $r.Description = "DZSM Reports"
        if (Test-PathFast $iconReports) { $r.IconLocation = $iconReports }
        $r.Save()
        Append-Line $ShortcutLogFile "OK Ordner-Link erstellt: Reports"
    }

    $buildTarget = Join-Path $BuildWrapperRoot "DeutschZ_Build_Wrapper_CheckOnly.bat"
    if (Test-PathFast $buildTarget) {
        $b = $shell.CreateShortcut((Join-Path $desktopFolder "Build Wrapper - Check.lnk"))
        $b.TargetPath = $buildTarget
        $b.WorkingDirectory = $BuildWrapperRoot
        $b.Description = "DeutschZ Build Wrapper CheckOnly"
        if (Test-PathFast $iconBuild) { $b.IconLocation = $iconBuild }
        $b.Save()
        Append-Line $ShortcutLogFile "OK Ordner-Link erstellt: Build Wrapper - Check"
    }
...
function Update-GitMergeDiagnostics {
    "DeutschZ System Manager v0.4.4 - Git Merge Diagnostics" | Out-File $GitMergeStatusFile -Encoding UTF8
    ...
    "Zweck: GitHub Desktop Merge-Konflikte auswerten, damit Patrick nicht blind Current/Incoming waehlt." | Out-File $GitMergeStatusFile -Append -Encoding UTF8
    "" | Out-File $GitMergeStatusFile -Append -Encoding UTF8

    if (-not (Test-PathFast $DeutschZRepoRoot)) {
        "FEHLER: RepoRoot fehlt." | Out-File $GitMergeStatusFile -Append -Encoding UTF8
        return
    }

    if (-not (Test-PathFast (Join-Path $DeutschZRepoRoot ".git"))) {
        "FEHLER: Kein .git Ordner im RepoRoot gefunden." | Out-File $GitMergeStatusFile -Append -Encoding UTF8
        return
    }
    ...
    try {
        "---- git branch --show-current ----" | Out-File $GitMergeStatusFile -Append -Encoding UTF8
        git branch --show-current 2>&1 | Out-File $GitMergeStatusFile -Append -Encoding UTF8

        "" | Out-File $GitMergeStatusFile -Append -Encoding UTF8
        "---- git status --short --branch ----" | Out-File $GitMergeStatusFile -Append -Encoding UTF8
        git status --short --branch 2>&1 | Out-File $GitMergeStatusFile -Append -Encoding UTF8

        "" | Out-File $GitMergeStatusFile -Append -Encoding UTF8
        "---- git status ----" | Out-File $GitMergeStatusFile -Append -Encoding UTF8
        git status 2>&1 | Out-File $GitMergeStatusFile -Append -Encoding UTF8

        "" | Out-File $GitMergeStatusFile -Append -Encoding UTF8
        "---- Merge/Cherry/Rebase State ----" | Out-File $GitMergeStatusFile -Append -Encoding UTF8
        ...
        foreach ($stateFile in @("MERGE_HEAD","MERGE_MSG","MERGE_MODE","REBASE_HEAD","CHERRY_PICK_HEAD")) {
            ...
            if (Test-PathFast $p) {
                "STATE: $stateFile vorhanden" | Out-File $GitMergeStatusFile -Append -Encoding UTF8
                try { Get-Content $p -ErrorAction SilentlyContinue | Out-File $GitMergeStatusFile -Append -Encoding UTF8 } catch {}
            } else {
                "STATE: $stateFile nicht vorhanden" | Out-File $GitMergeStatusFile -Append -Encoding UTF8
            }
        }

        "" | Out-File $GitMergeStatusFile -Append -Encoding UTF8
        "---- Unmerged files: git diff --name-only --diff-filter=U ----" | Out-File $GitMergeStatusFile -Append -Encoding UTF8
        $unmerged = @(git diff --name-only --diff-filter=U 2>$null)
        if ($unmerged.Count -eq 0) {
            "Keine unmerged/conflicted files laut git diff." | Out-File $GitMergeStatusFile -Append -Encoding UTF8
        } else {
            $unmerged | Out-File $GitMergeStatusFile -Append -Encoding UTF8
        }

        "DeutschZ System Manager v0.4.4 - Git Conflict Files" | Out-File $GitConflictFilesFile -Encoding UTF8
        "Datum: $(Get-Date)" | Out-File $GitConflictFilesFile -Append -Encoding UTF8
        "" | Out-File $GitConflictFilesFile -Append -Encoding UTF8

        "---- git ls-files -u ----" | Out-File $GitConflictFilesFile -Append -Encoding UTF8
        git ls-files -u 2>&1 | Out-File $GitConflictFilesFile -Append -Encoding UTF8

        "" | Out-File $GitConflictFilesFile -Append -Encoding UTF8
        "---- Unmerged Datei-Liste ----" | Out-File $GitConflictFilesFile -Append -Encoding UTF8
        if ($unmerged.Count -gt 0) {
            foreach ($f in $unmerged) {
                "KONFLIKT: $f" | Out-File $GitConflictFilesFile -Append -Encoding UTF8
            }
        } else {
            "Keine Konfliktdateien laut git." | Out-File $GitConflictFilesFile -Append -Encoding UTF8
        }

        "DeutschZ System Manager v0.4.4 - Git Conflict Marker Context" | Out-File $GitConflictMarkersFile -Encoding UTF8
        "Datum: $(Get-Date)" | Out-File $GitConflictMarkersFile -Append -Encoding UTF8
        "Hinweis: HEAD/CURRENT/main = lokaler Stand. Incoming/origin/main = GitHub-Stand." | Out-File $GitConflictMarkersFile -Append -Encoding UTF8
        "" | Out-File $GitConflictMarkersFile -Append -Encoding UTF8
        ...
        if ($unmerged.Count -gt 0) {
            foreach ($f in $unmerged) {
                ...
                "" | Out-File $GitConflictMarkersFile -Append -Encoding UTF8
                "============================================================" | Out-File $GitConflictMarkersFile -Append -Encoding UTF8
                "DATEI: $f" | Out-File $GitConflictMarkersFile -Append -Encoding UTF8
                "============================================================" | Out-File $GitConflictMarkersFile -Append -Encoding UTF8

                if (-not (Test-PathFast $full)) {
                    "FEHLT lokal: $full" | Out-File $GitConflictMarkersFile -Append -Encoding UTF8
                    continue
                }
                ...
                try {
                    ...
                    if ($markerIndexes.Count -eq 0) {
                        "Keine Konfliktmarker im Text gefunden. Eventuell bereits geloest, aber noch nicht als resolved markiert." | Out-File $GitConflictMarkersFile -Append -Encoding UTF8
                    } else {
                        "Marker-Anzahl: $($markerIndexes.Count)" | Out-File $GitConflictMarkersFile -Append -Encoding UTF8
                        foreach ($idx in $markerIndexes) {
                            $start = [Math]::Max(0, $idx - 8)
                            $end = [Math]::Min($lines.Count - 1, $idx + 16)
                            "" | Out-File $GitConflictMarkersFile -Append -Encoding UTF8
                            "---- Kontext um Zeile $($idx+1) ----" | Out-File $GitConflictMarkersFile -Append -Encoding UTF8
                            for ($j=$start; $j -le $end; $j++) {
                                ("{0,6}: {1}" -f ($j+1), $lines[$j]) | Out-File $GitConflictMarkersFile -Append -Encoding UTF8
                            }
                        }
                    }
                } catch {
                    "FEHLER beim Lesen: $($_.Exception.Message)" | Out-File $GitConflictMarkersFile -Append -Encoding UTF8
                }
            }
        }
...
Stand: $(Get-Date)
Repo: $DeutschZRepoRoot

## Bedeutung

main / HEAD / Current Change = dein lokaler Stand auf dem PC.
origin/main / Incoming Change = Stand von GitHub.

## Deine aktuelle Absicht

Wenn der PC-Stand wirklich der aktuelle Masterstand sein soll:
- In Konfliktdateien meistens `Accept Current Change` waehlen.
- Nicht `Accept Incoming`, wenn GitHub alte Versionen enthaelt.
- Danach speichern.
- Erst wenn alle Konflikte geloest sind: Continue merge.

## Nicht blind machen

Nicht auf Continue merge druecken, solange Konflikte offen sind.
Nicht origin/main nehmen, wenn lokale Fixes aktueller sind.
Nicht Workshopdateien anfassen, ausser Patrick fordert es ausdruecklich an.

## Sicherste Vorgehensweise

1. Uploadpaket hochladen.
2. ChatGPT prueft 30/31/32 Reports.
3. Dann Entscheidung:
   - alles Current/main behalten
   - einzelne Dateien manuell mischen
   - Merge abbrechen

## Wenn du jetzt dringend weitermachen musst

Bei DeutschZModZ ist in der Regel `main / Current / HEAD` der PC-Stand.
Wenn du sicher bist, dass dein PC aktueller ist, kannst du in VS Code bei allen Konflikten `Accept Current Change` waehlen.
Danach speichern und GitHub Desktop `Continue merge`.

## Nach dem Merge

Danach DZSM erneut ausfuehren und Uploadpaket hochladen.
"@
        $guide | Out-File -FilePath $GitMergeGuideFile -Encoding UTF8

    } catch {
        "FEHLER bei Git Merge Diagnostics: $($_.Exception.Message)" | Out-File $GitMergeStatusFile -Append -Encoding UTF8
    } finally {
        Pop-Location
    }
}
...
function Update-UploadPackage {
    "DeutschZ System Manager v0.4.4 - Upload-Paket FINAL" | Out-File $UploadPackageLogFile -Encoding UTF8
