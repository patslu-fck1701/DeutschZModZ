...
class GroundZeroConfig
{
    ...
    static GroundZeroConfig Load()
    {
        ...
        {
            if (FileExist(GroundZeroConstants.LEGACY_CONFIG_PATH))
            {
                JsonFileLoader<GroundZeroConfig>.JsonLoadFile(GroundZeroConstants.LEGACY_CONFIG_PATH, cfg);
                GroundZeroLogging.Info("Config", "Imported legacy config from root profile path");
            }
            cfg.Save();
            GroundZeroLogging.Info("Config", "Created or migrated default config");
            return cfg;
        }
