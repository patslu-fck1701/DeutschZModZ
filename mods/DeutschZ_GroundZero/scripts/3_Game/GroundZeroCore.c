...
class GroundZeroCore
{
    ...
    ref GroundZeroEventManager EventManager;
    ...
    void InitServer()
    {
        ...
        EventManager = new GroundZeroEventManager(Config, State, Notifications);
        EventManager.RestoreOrIdle();
        m_Initialized = true;
    ...
    void Tick()
    {
        ...
        if (EventManager) EventManager.Tick();
    }

    void StartEventAdmin()
    {
        if (EventManager) EventManager.StartEvent();
    }

    void OnPlayerKilled(PlayerBase player, Object killer)
    {
        if (EventManager && player) EventManager.OnPlayerKilled(player, killer);
    }

    void RequestRetry(PlayerBase player)
    {
        if (EventManager && player) EventManager.RequestRetry(player);
    }

    void RequestAbort(PlayerBase player)
    {
        if (EventManager && player) EventManager.RequestAbort(player);
    }
