...
class CourierZConfig
{
    ...
    static CourierZConfig Load()
    {
        ...
        {
            if (FileExist(CourierZConstants.LEGACY_CONFIG_PATH))
            {
                JsonFileLoader<CourierZConfig>.JsonLoadFile(CourierZConstants.LEGACY_CONFIG_PATH, cfg);
                DeutschZCore_Log.Info(CourierZConstants.LOG_SCOPE, "imported legacy CourierZ config from root profile path");
            }
            cfg.Save();
            DeutschZCore_Log.Info(CourierZConstants.LOG_SCOPE, "created or migrated default CourierZ config");
            return cfg;
        }
